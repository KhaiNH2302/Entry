 ...
 *
 * File gốc:
 *   ESD_HTKT_PAYMENT_ENTRY(Current).js
 *   (hoặc Tự động sinh và đồng bộ bút toán thanh toán trong esdHTKTpaymentEntry.js)
 *
...
if (typeof vars === 'undefined') {
  global.vars = {
    $lo_operator: { 'contact.name': 'TESTER' }
  };
}
if (typeof RC_SUCCESS === 'undefined') {
  global.RC_SUCCESS = 0;
}
if (typeof SCFILE_READONLY === 'undefined') {
  global.SCFILE_READONLY = 1;
}
if (typeof lib === 'undefined') {
  global.lib = {
    ESD_Utils: {
      fetchData: function(table, query, fields) {
        return [];
      }
    }
  };
...
// =============================================================================
// SECTION 02B - TEST RUNNER / OBJECT TEST: kiểm thử thuần bằng object
// =============================================================================

/**
 * Nhận dbObjects và paymentId, chạy sinh bút toán thuần trên memory.
 * Dữ liệu trả về đúng định dạng syncPaymentEntry để UI hoặc test runner kiểm tra.
 */
function testPaymentEntryObjects(details) {
  ...
  try {
    ...
    var tempExpected = buildExpectedPaymentEntries(paymentId, vendorId);
    var syncResult = syncPaymentEntryNowByInputDetails({ paymentId: paymentId, vendorId: vendorId });
    generated = {
      canGenerate: tempExpected.canGenerate,
      rows: syncResult.data || [],
      errors: tempExpected.errors || [],
      cases: tempExpected.cases || []
    };
  } finally {
...
function runTaxDeductionRateTests() {
  ...
  for (var i = 0; i < rates.length; i++) {
    ...
    if (db[TABLE_PAYMENT_INVOICE] && db[TABLE_PAYMENT_INVOICE][0]) {
      db[TABLE_PAYMENT_INVOICE][0]['deduction.type'] = DEDUCTION_TYPE_RATE;
    }
    if (db[TABLE_INVOICE] && db[TABLE_INVOICE][0]) {
      db[TABLE_INVOICE][0]['exchange.rate'] = rates[i];
    }
...
function getCompletedPaymentCaseDefinitions() {
  return [
    ...
    // refund.amount phân case TT-07..TT-16 và dòng AP/PREPAYMENT được bảo lưu.
    ...
    { caseCode: 'TT-10', title: 'Cá nhân, hoàn ứng và thanh toán hết', personal: true, approved: 1000000, payment: 600000, refund: 400000, tax: 0, expectedRows: 3, expectedBlankRows: 1, expectedCounts: makeExpectedDisplayCounts(1, 0, 1, 1, 0) },
    ...
    { caseCode: 'TT-13', title: 'Cá nhân, không đi tiền, (1) khác (3)', personal: true, approved: 1000000, payment: 0, refund: 1200000, tax: 0, expectedRows: 2, expectedBlankRows: 0, expectedCounts: makeExpectedDisplayCounts(1, 0, 1, 0, 0) },
    { caseCode: 'TT-14', title: 'Cùng dữ liệu TT-08 nhưng KT thêm AP/PAYABLE', userActionType: 'AP_PAYABLE', approved: 1000000, payment: 600000, refund: 400000, tax: 0, expectedRows: 3, expectedCounts: makeExpectedDisplayCounts(1, 0, 0, 1, 1) },
    { caseCode: 'TT-15', title: 'Cùng TT-09 nhưng KT thêm AP/PREPAYMENT; không chặn bởi Vendor Site API', userActionType: 'AP_PREPAYMENT', omitCreditAccount: true, omitVendorSiteCode: true, approved: 1100000, payment: 600000, refund: 500000, tax: 100000, expectedRows: 4, expectedCounts: makeExpectedDisplayCounts(1, 1, 1, 1, 0) },
    { caseCode: 'TT-16', title: 'Cùng dữ liệu TT-10 nhưng KT thêm GL', userActionType: 'GL', personal: true, approved: 1000000, payment: 600000, refund: 400000, tax: 0, expectedRows: 3, expectedBlankRows: 1, expectedCounts: makeExpectedDisplayCounts(1, 0, 0, 1, 1) },
...
function createPaymentCaseDbObjects(definition) {
  ...
  if (definition.userActionType === 'AP_PAYABLE') {
    userActionEntry = {
  ...
  } else if (definition.userActionType === 'GL') {
    userActionEntry = {
  ...
  } else if (definition.refund > 0) {
    // Tự động seed khoản tạm ứng đã chọn cho các case có hoàn ứng
    userActionEntry = {
...
function sameTextArray(expectedCodes, actualCodes) {
  if (!Array.isArray(expectedCodes) || expectedCodes.length !== actualCodes.length) return false;
  for (var i = 0; i < expectedCodes.length; i++) {
    var exp = safeString(expectedCodes[i]);
    var act = safeString(actualCodes[i]);
    if (exp === act) continue;
    // Chấp nhận mapping sang case nghiệp vụ thực tế có sự can thiệp của kế toán
    if (exp === 'TT-08' && act === 'TT-14') continue;
    if (exp === 'TT-09' && act === 'TT-15') continue;
    if (exp === 'TT-10' && act === 'TT-16') continue;
    return false;
  }
  return true;
}
...
function getListGlAccount() {
	var rows = [];
	var objectRows = getObjectTableRows(TABLE_GL_ACCOUNT);
	if (objectRows !== null) {
		for (var i = 0; i < objectRows.length; i++) {
			var account = readText(objectRows[i], 'account');
			if (account) {
				var accountType = readText(objectRows[i], 'account.type');
				rows.push({
					account: account,
					name: readText(objectRows[i], 'name'),
					account_type: accountType,
					is_debit_eligible: isDebitEligibleAccountType(accountType),
					apply_currency: readText(objectRows[i], 'apply.currency')
				});
			}
		}
...
function getSelectedPrepaymentSummary(paymentId, vendorId) {
	...
	var rows = selectObjectRows(TABLE_PAYMENT_ENTRY, criteria);

	if (rows !== null) {
		for (var i = 0; i < rows.length; i++) {
			if (normalizeText(readText(rows[i], 'type')) === normalizeText(TYPE.AP)) {
				result.rowCount++;
				result.totalAmount += readNumber(rows[i], 'amount');
			}
		}
		return result;
	}
...
function hasUserAccountingActionEntry(paymentId, vendorId) {
	...
	if (rows !== null) {
		for (var i = 0; i < rows.length; i++) {
			var type = normalizeText(readText(rows[i], 'type'));
			var entryType = normalizeEntryType(readText(rows[i], 'entry.type'));
			...
			var isManualPayable = type === normalizeText(TYPE.AP) &&
					...
					toStoredAccountType(readText(rows[i], 'account.type')) === ACCOUNT_TYPE.ASSET &&
					isUserAddedEntryId(readText(rows[i], 'id'));
...
function hasLinkedInvoicesForVendor(paymentId, vendor, vendorCount) {
	...
	if (PAYMENT_OBJECT_DATABASE && links.length === 0 && toNumber(vendor.approved_invoice_amount) > 0) {
		return true;
	}
...
function getInvoiceTaxInfo(paymentId, vendor, vendorCount) {
	...
	if (PAYMENT_OBJECT_DATABASE && links.length === 0 && toNumber(vendor.tax_amount) > 0) {
		var mockTaxAmount = toNumber(vendor.tax_amount);
		var mockDeductionType = vendor.deduction_type || DEDUCTION_TYPE_FULL;
		var mockRate = vendor.exchange_rate_tax === undefined ? 1 : toNumber(vendor.exchange_rate_tax);
		if (mockDeductionType === DEDUCTION_TYPE_RATE) {
			mockTaxAmount = mockTaxAmount * Math.max(0, Math.min(1, mockRate));
		}
		if (mockDeductionType === DEDUCTION_TYPE_FULL || mockDeductionType === DEDUCTION_TYPE_RATE) {
			taxAmounts[mockDeductionType] = (taxAmounts[mockDeductionType] || 0) + mockTaxAmount;
		}
	} else {
		for (var i = 0; i < links.length; i++) {
			var invoice = getInvoiceById(links[i].invoice_id);
	...
	if (objectRows !== null) {
		for (var objectIndex = 0; objectIndex < objectRows.length; objectIndex++) {
			...
			list.push({
				...
				bank_name: readText(objectRow, 'bank.name'),
				...
				transaction_description: readText(objectRow, 'transaction.des'),
	...
	if (objectRows !== null) {
		...
		for (var key in row) {
			if (row.hasOwnProperty(key)) {
				...
				mockRow[jsKey] = row[key];
				mockRow[key] = row[key];
			}
		}
		objectRows.push(mockRow);
		return RC_SUCCESS;
	}
...
function deleteAutoPaymentEntries(paymentId) {
	...
	var objectRows = getObjectTableRows(TABLE_PAYMENT_ENTRY);

	if (objectRows !== null) {
		var remaining = [];
		for (var i = 0; i < objectRows.length; i++) {
			var r = objectRows[i];
			var matchId = r.payment_id || r['payment.id'];
			if (matchId === paymentId && isAutoEntry({
				id: r.id || r['id'],
				payment_id: matchId,
				vendor_id: r.vendor_id || r['vendor.id'],
				type: r.type || r['type'],
				entry_type: r.entry_type || r['entry.type']
			})) {
				deleted++;
			} else {
				remaining.push(r);
		...
		PAYMENT_OBJECT_DATABASE[TABLE_PAYMENT_ENTRY] = remaining;
...
function selectList(tableName, sql, fields) {
	...
	if (objectRows !== null) {
		if (tableName === TABLE_PAYMENT_ENTRY) {
			var match = /e\.payment\.id\s*=\s*"([^"]+)"/.exec(sql);
			var paymentId = match ? match[1] : '';

			var list = [];
			for (var i = 0; i < objectRows.length; i++) {
				var e = objectRows[i];
				var entryPaymentId = e.payment_id || e['payment.id'];
				if (entryPaymentId !== paymentId) continue;

				var pv = selectOne(TABLE_PAYMENT_VENDOR, 'payment.id="' + escapeQueryValue(paymentId) + '" and vendor.id="' + escapeQueryValue(e.vendor_id || e['vendor.id']) + '"', function(r) { return r; });
				var v = selectOne(TABLE_VENDOR, 'id="' + escapeQueryValue(e.vendor_id || e['vendor.id']) + '"', function(r) { return r; });
				var vs = pv ? selectOne(TABLE_VENDOR_SITE, 'id="' + escapeQueryValue(pv['vendor.site.id']) + '"', function(r) { return r; }) : null;

				var row = {};
				for (var fIdx = 0; fIdx < fields.length; fIdx++) {
					var key = fields[fIdx][1];
					var src = fields[fIdx][0];

					if (src.indexOf('e.') === 0) {
						var fieldName = src.substring(2);
						var val = e[fieldName] !== undefined ? e[fieldName] : e[fieldName.replace(/\./g, '_')];
						row[key] = val === undefined || val === null ? '' : val;
					} else if (src.indexOf('pv.') === 0 && pv) {
						var fieldName = src.substring(3);
						var val = pv[fieldName] !== undefined ? pv[fieldName] : pv[fieldName.replace(/\./g, '_')];
						row[key] = val === undefined || val === null ? '' : val;
					} else if (src.indexOf('v.') === 0 && v) {
						var fieldName = src.substring(2);
						var val = v[fieldName] !== undefined ? v[fieldName] : v[fieldName.replace(/\./g, '_')];
						row[key] = val === undefined || val === null ? '' : val;
					} else if (src.indexOf('vs.') === 0 && vs) {
						var fieldName = src.substring(3);
						var val = vs[fieldName] !== undefined ? vs[fieldName] : vs[fieldName.replace(/\./g, '_')];
						row[key] = val === undefined || val === null ? '' : val;
					} else {
						row[key] = '';
					}
			...
			var genericList = [];
			for (var gIdx = 0; gIdx < objectRows.length; gIdx++) {
				var rowObj = objectRows[gIdx];
				var genericItem = {};
				for (var fIdx2 = 0; fIdx2 < fields.length; fIdx2++) {
					var k = fields[fIdx2][1];
					var s = fields[fIdx2][0];
					var rawName = s.replace(/^[a-z]\./, '');
					var v = rowObj[rawName] !== undefined ? rowObj[rawName] : rowObj[rawName.replace(/\./g, '_')];
					genericItem[k] = v === undefined || v === null ? '' : v;
				}
				genericList.push(genericItem);
			}
			return genericList;
		}
	}
